   ...
   if  not defUnit:IsDead() and attUnit:IsHasPromotion(NanSongSeaUnitID) then
	...
	and attUnit:IsHasPromotion(BeiWeiCavalryID) then
		...
		if defUnit:IsDead() then
			attUnit:ChangeDamage(0 - 0.15 * maxattUnitHP)
	...
	if not attUnit:IsDead() and  attUnit:IsHasPromotion(BeiWeiCavalryID) then
		...
		local aCS = GameInfo.Units[attUnit:GetUnitType()].Combat
		iCombat = aCS * (1 + combatfactor)
		attUnit:SetBaseCombatStrength(iCombat)
		print("attUnit:SetBaseCombatStrength(iCombat)")
	end

	if defUnit ~= nil and not defUnit:IsDead() and  defUnit:IsHasPromotion(BeiWeiCavalryID) then
		local iCombat = 0
		local defUnitHP = defUnit:GetCurrHitPoints() 
		local maxdefUnitHP = defUnit:GetMaxHitPoints()
		local factor = 1 - defUnitHP / maxdefUnitHP + 0.01
		local combatfactor = math.tan(math.pi * factor / 2)
		local aCS = GameInfo.Units[defUnit:GetUnitType()].Combat
		iCombat = aCS * (1 + combatfactor)
		defUnit:SetBaseCombatStrength(iCombat)
		print("defUnit:SetBaseCombatStrength(iCombat)")
	end
    ...
    for i = 0, unitCount-1, 1 do
        local pFoundUnit = batPlot:GetUnit()
        if pFoundUnit and 
        (pFoundUnit:IsHasPromotion(AntiAirID) or pFoundUnit:GetDomainType() == DomainTypes.DOMAIN_AIR or 
            ...
            if (adjPlot ~= nil) then
                ...
                if unitCount > 0 then
                    for i = 0, unitCount-1, 1 do
                        ...
                        if pFoundUnit and 
                        (pFoundUnit:IsHasPromotion(AntiAirID) or 
						pFoundUnit:GetDomainType() == DomainTypes.DOMAIN_AIR or 
						pFoundUnit:IsHasPromotion(NavalRangedCruiserUnitID) or 
						pFoundUnit:IsHasPromotion(DestroyerID)) then
                            pFoundUnit:SetMoves(0);
